...
export function AssessmentSummary({
  ...
  if (
    !jobTypes.some((jt) => !jt.includes('New Build')) ||
    jobTypes.length === 0
  ) {
    return null;
  }
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
          ...
          <div className="space-y-2 text-sm">
            {Object.entries(jobSpecificAnswers)
              .filter(
                ([key]) =>
                  key.includes('current_') && key.includes('_condition')
              )
              .map(([key, value]) => {
            ...
            {Object.entries(jobSpecificAnswers).filter(
              ([key]) => key.includes('current_') && key.includes('_condition')
            ).length === 0 && (
              <p className="text-blue-600 italic">
                Complete current condition assessments to see summary
              </p>
            )}
          </div>
        </div>
        ...
        <div className="bg-green-50 p-4 rounded-lg border border-green-200">
          ...
          <div className="space-y-2 text-sm">
            ...
            {Object.entries(jobSpecificAnswers).filter(
              ([key, value]) => key.includes('recommended_') && value
            ).length === 0 && (
              <p className="text-green-600 italic">
                Complete recommendations to see summary
              </p>
            )}
          </div>
        </div>
